...
function loadSlangDataset() {
    try {
        ...
        if (!fs.existsSync(slangFilePath)) {
            console.log('  slang_data.json not found. Using fallback slang terms...');
        ...
        console.log(` Using minimal fallback terms`);
    }
}
...
function loadRedditPostsAndComments() {
    try {
        ...
        // Show some sample subreddits for debugging
        ...
        redditPosts = createFallbackPosts();
        realComments = createFallbackComments();
        console.log(`  Using ${redditPosts.length} fallback posts`);
    }
}
...
// Human response templates based on AI constraint level fake scenarios
...
// Vote on post or comment - this is a simulation imp to simulate voting behavior
